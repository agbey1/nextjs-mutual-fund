...
import { mockMembers, mockExpenses, mockTransactions, mockAuditLogs } from '../lib/mockData';
...
async function generateSQL() {
    ...
    for (const m of mockMembers) {
        ...
        const safe = (str: any) => str ? `'${String(str).replace(/'/g, "''")}'` : 'NULL';
    ...
    // Standalone
    mockTransactions.forEach((t: any) => allTxs.set(t.id, { ...t, memberId: t.memberId || 'UNKNOWN' }));

    // Embedded
    mockMembers.forEach((m: any) => {
        if (m.transactions) {
            m.transactions.forEach((t: any) => {
                allTxs.set(t.id, { ...t, memberId: m.id });
            });
        }
    });
